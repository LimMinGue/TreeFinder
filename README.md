# TreeFinder

> Windows 탐색기의 계보를 잇는 macOS 네이티브 파일 매니저 — 왼쪽 폴더 트리 + 파일 목록, 폴더는 항상 위, 모든 컬럼 정렬

![platform](https://img.shields.io/badge/platform-macOS%2014%2B-blue)
![swift](https://img.shields.io/badge/Swift-AppKit%20(programmatic)-orange)
![license](https://img.shields.io/badge/license-Proprietary-lightgrey)

TreeFinder는 "폴더 트리를 항상 옆에 두고 쓰는" Windows 탐색기식 탐색 경험을 macOS에서 네이티브로 재현한 파일 매니저입니다. Finder가 잘하는 것(Quick Look·태그 색 표시·SMB 네트워크 브라우즈)과 탐색기가 잘하던 것(트리 탐색·전 컬럼 정렬·폴더 크기 표시)을 한 창에 담고, 파일 목록 옆에 붙은 터미널이 목록과 서로를 따라갑니다.

> ⚠️ **개인 프로젝트입니다.** ad-hoc 서명 로컬 빌드 전용이며(공증·배포 없음), 개인 정보를 일절 수집하지 않습니다.

![TreeFinder — 트리·파일 목록·마크다운 미리보기 (다크 모드)](docs/screenshots/main.png)

<p align="center"><sub>왼쪽부터: 즐겨찾기·위치(네트워크·휴지통 포함) 사이드바 트리 · 파일 목록 · 미리보기 패널(마크다운 WYSIWYG 렌더 + 파일 정보 테이블)</sub></p>

---

## 📸 스크린샷

**정보 가져오기 (⌘I)** — Finder식 정보 창. 폴더는 실측 크기(백그라운드 계산), 이미지는 EXIF, 권한 표까지 표시합니다.

<img src="docs/screenshots/get-info.png" alt="정보 가져오기 창" width="360">

---

## ✨ 주요 기능

### 탐색
- 🌳 **폴더 트리 + 파일 목록** — 탐색기식 2단 구조, 트리가 탐색을 따라 펼쳐짐(Expand to Open Folder)
- 🗂 **탭** (⌘T) — 탭별 독립 히스토리·정렬·뷰 스타일, 탭 위 드래그 앤 드롭(스프링 오픈)
- 🔀 **듀얼 페인** (⇧⌘D) — 독립 브라우저 2개 나란히, 페인 간 드래그로 이동/복사
- 🧭 **하단 경로 바** — 로컬라이즈드 브레드크럼, 세그먼트 클릭 즉시 이동 · ⌘L 주소 입력 · ⇧⌘G 폴더로 이동
- 🌐 **네트워크 브라우즈** — 사이드바 네트워크 클릭 = 주변 SMB 컴퓨터 발견(Bonjour), 더블클릭 = 연결(Finder식). 마운트한 공유 자동 기억·원클릭 재연결·인라인 ⏏
- 🗑 **휴지통·볼륨** — 사이드바에서 바로 접근, USB/네트워크 마운트 자동 반영, **휴지통 비우기**(⇧⌘⌫)
- ⌨️ **Finder 단축키 그대로** — ⇧⌘H 홈 · ⇧⌘O 문서 · ⌥⌘L 다운로드 · ⇧⌘A 응용 프로그램 · ⇧⌘U 유틸리티 · ⇧⌘C 컴퓨터 · ⇧⌘I iCloud Drive · ⌘N 새 윈도우 · ⌘D 복제 · ⌃⌘N 선택 항목으로 새 폴더
- 🔎 **검색 범위** — 돋보기 메뉴에서 이 폴더 / 이 Mac(Spotlight), 결과 우클릭 **"들어있는 폴더 열기"**

### 보기
- 👀 **리스트 · 아이콘 · 갤러리 뷰** (⌥⌘1/2/3) — 갤러리 = 대형 미리보기 + 필름스트립
- ↕️ **전 컬럼 정렬** — 이름·수정일·생성일·크기·종류, 폴더 우선 불변 · **헤더 구분선 더블클릭 = 컬럼 적정 폭**
- 📁 **폴더 크기 표시** — 백그라운드 재귀 계산, 크기 정렬에 폴더 참여, 부분 측정은 "≥" 정직 표기
- 🏷 **태그 풀 로우 컬러** — Finder 라벨 색을 행 전체 배경으로, 트리에도 태그 색 반영
- 🫥 **숨김 파일 토글** (⇧⌘.) — 목록과 트리가 같은 설정 공유

### 미리보기 · 편집
- 👁 **미리보기 패널** — QuickLook + 파일 정보 테이블(이미지는 EXIF: 카메라·조리개·ISO 등), 핀치 확대/축소
- 📃 **내용 기반 텍스트 미리보기** — 확장자가 아니라 **내용**으로 판정: `.gitignore`·`LICENSE`·`.yml` 등 시스템이 못 여는 파일도 일반 텍스트(UTF-8)면 그대로 표시
- 📄 **HWP/HWPX 미리보기** — 한글 문서를 전 페이지 렌더([rhwp](https://github.com/edwardkim/rhwp) 0.8.6 동반). 한컴 뷰어를 깔아도 Finder는 못 보여 주는 것(이 기기 실측: Quick Look 생성기·Spotlight 색인기 없음)
- 🔎 **HWP/HWPX 본문 검색** — Spotlight가 색인하지 않는 한글 문서 본문을 검색에 포함(이 폴더·이 Mac, 상한 500개, 결과 캐시)
- 📋 **HWP 본문 텍스트 복사 · PDF로 내보내기** — 우클릭. 한컴 없이 문서 내용을 붙여 넣고 PDF로 만듭니다(충실도는 rhwp 렌더 기준)
- ✏️ **편집은 오픈소스 [HOP](https://golbin.github.io/hop/)** (`brew install hop`) 추천 — 설치하면 Open With에 자동으로 뜨고, HOP이 함께 설치하는 Quick Look 확장으로 갤러리 썸네일·스페이스바 미리보기도 살아납니다
- ✍️ **마크다운 WYSIWYG 편집** — .md 선택 즉시 렌더+편집, ⌘S 저장([Toast UI Editor](https://ui.toast.com/tui-editor) 오프라인 동반)

### 내장 터미널 ([SwiftTerm](https://github.com/migueldeicaza/SwiftTerm))
- ⌨️ **멀티 탭** — 미리보기 자리 탭 전환, 독립 셸 세션 여러 개(파일 탭과 같은 필 디자인), 전환해도 세션 유지
- 🏷 **자동 탭 제목 (iTerm2식)** — 실행 중 명령·현재 경로를 탭에 표시, **더블클릭 = 이름 직접 지정**(세션 한정)
- 🛡 **닫기 확인** — 탭 ✕는 확인 후 종료(실행 중 작업 보호, 기본 버튼 = 취소)
- 📥 **파일 드롭 = 경로 입력** — 파일 목록에서 끌어다 놓으면 이스케이프된 전체 경로 입력(Terminal.app 규약, 실행 없음)
- 📖 **명령 도움말 밴드** — 하단에 기본 안내 상시 표시, `vi`·`grep`·`tar` 등 기본 명령 실행 시 한국어 치트시트로 자동 교체(크기 조절 가능)
- 📂 **"현재 폴더로 이동" = 항상 새 탭** — 새 셸에서 cd 실행(vi 등 실행 중 프로그램에 명령이 주입되는 오염 원천 차단)
- 🔤 **터미널 폰트 설정** — 고정폭 패밀리·크기 지정(Nerd Font 파워라인 글리프 지원), ⌘= / ⌘- 즉시 조절
- 🔍 **스크롤백 찾기** — ⌘F(정규식·전체 단어)·⌘G / ⇧⌘G, 스크롤백 10,000줄(설정 1천~10만)
- 🔗 **출력 속 경로 ⌘클릭 = 파일 목록에서 선택·미리보기** — `파일:줄` 표기·상대 경로도 인식
- 🧭 **셸 cd 따라가기** (설정, 기본 꺼짐) — 터미널에서 `cd` 하면 파일 목록과 트리가 그 폴더로
- 📎 **선택 항목 → 터미널 인자** — ⌥⌘⏎ 또는 우클릭 "터미널에 경로 붙여넣기"(실행은 사용자 Enter)
- 🌳 **명령 실행 후 트리 자동 갱신** · 💾 **재실행 시 터미널 탭(폴더·이름) 복원** · ⌘T / ⌘W / ⌘K가 터미널 포커스에선 터미널 탭·버퍼에 적용

### 파일 작업
- 🧰 복사/이동/휴지통/복제/압축(zip)/인라인 이름 변경 — **Undo 지원**, 진행률 패널+취소
- ⚖️ **같은 이름 충돌 시 선택** — 대치 · 둘 다 유지 · 건너뛰기 · 폴더 **병합**, "모두 적용". 대치한 원본은 휴지통으로 가므로 ⌘Z로 되돌릴 수 있음
- ♻️ **되돌려 놓기** — TreeFinder가 지운 파일만 원위치로 복원(다른 앱이 지운 파일은 Finder에서)
- ℹ️ **정보 가져오기** (⌘I) — Finder식 정보 창(폴더 실측 크기·위치·EXIF·권한 표시), **잠금·확장자 가리기 편집**(그 외 읽기 전용)
- 🅰️ **기본 앱 지정** — Open With에서 ⌥ 누르면 "항상 이 앱으로 열기"(같은 종류 전체)
- 📌 **Drop Stack** — 사이드바 임시 선반에 파일을 모아뒀다 목적지에서 일괄 이동/복사
- 🔤 **클립보드 NFC 보정** — 경로 복사 시 한글 자모 분리(ㅎㅏㄴㄱㅡㄹ) 방지

그 밖에: FSEvents 자동 새로고침 · 다크 모드 · 한국어/영어 · Open With · 공유 · Finder에 보기

### 🚫 하지 않는 것 (범위 밖)
iCloud 동기화 상태 표시 · 원격 프로토콜(SFTP/FTP/WebDAV) · tmux 통합 · 빠른 동작/서비스 메뉴 · 다른 앱이 지운 파일의 되돌려 놓기(Finder 비공개 포맷) · 리스트 뷰 인라인 펼침 삼각형(왼쪽 트리가 그 역할) · HWP 편집(HOP 등 전용 앱에 위임) · 암호화·배포용 HWP의 본문 검색

---

## ⌨️ 주요 단축키

| 단축키 | 동작 | 단축키 | 동작 |
|---|---|---|---|
| ⌘T / ⌘W | 새 탭 / 탭 닫기 | ⌘I | 정보 가져오기 |
| ⌃⇥ / ⌘1…9 | 탭 전환 | ⌘R | 이름 변경 |
| ⇧⌘D | 듀얼 페인 | ⌘⌫ | 휴지통으로 이동 |
| ⌥⌘1/2/3 | 아이콘/리스트/갤러리 뷰 | ⇧⌘N | 새 폴더 |
| ⇧⌘. | 숨김 파일 표시 | ⌥⌘C | 경로 복사 (NFC 보정) |
| ⌘[ / ⌘] / ⌘↑ | 뒤로 / 앞으로 / 상위 폴더 | ⌘F | 검색(터미널 포커스면 스크롤백 찾기) |
| ⇧⌘H / ⇧⌘O / ⌥⌘L / ⇧⌘A | 홈 / 문서 / 다운로드 / 응용 프로그램 | ⇧⌘U / ⇧⌘C / ⇧⌘I / ⇧⌘K | 유틸리티 / 컴퓨터 / iCloud Drive / 네트워크 |
| ⌘N / ⌘D / ⌃⌘N | 새 윈도우 / 복제 / 선택 항목으로 새 폴더 | ⌥⌘⏎ | 선택 경로를 터미널에 입력 |
| ⌘T / ⌘W / ⌘K (터미널) | 새 터미널 탭 / 터미널 탭 닫기 / 버퍼 지우기 | ⌘G / ⇧⌘G / ⌘= / ⌘- (터미널) | 다음·이전 찾기 / 글자 크게·작게 |
| ⌘L / ⇧⌘G | 주소 입력 / 폴더로 이동 | ⇧⌘P | 미리보기 패널 토글 |
| ⌘K | 서버에 연결 | ⌘, | 설정 |

---

## 🧰 요구사항

- **macOS 14.0 (Sonoma) 이상**, **Apple Silicon(arm64) 전용** — Intel Mac은 지원하지 않습니다(동반 HWP 렌더러 rhwp도 arm64 빌드)
- 빌드 시: **Xcode 15+** (SwiftTerm 패키지 해석과 Metal 툴체인이 필요해 Command Line Tools만으로는 부족합니다)

---

## 🛠️ 빌드 & 설치

```bash
git clone https://github.com/LimMinGue/TreeFinder.git
cd TreeFinder

# 최초 1회: Metal 툴체인 설치 — SwiftTerm 셰이더 컴파일에 필요
# (Xcode 26+는 기본 미포함. "cannot execute tool 'metal'" 오류가 나면 이 단계를 빠뜨린 것)
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer \
xcodebuild -downloadComponent MetalToolchain

DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer \
xcodebuild -project TreeFinder.xcodeproj -scheme TreeFinder \
           -configuration Release -derivedDataPath build/dd build
```

빌드 후 응용 프로그램 폴더로 설치:

```bash
ditto build/dd/Build/Products/Release/TreeFinder.app /Applications/TreeFinder.app
open /Applications/TreeFinder.app
```

위 과정을 한 번에 하려면 저장소의 `build.sh`를 쓰세요:

```bash
./build.sh          # Release 빌드
./build.sh -d       # Debug 빌드
./build.sh -i -r    # 빌드 → /Applications 설치 → 실행
./build.sh -h       # 옵션 도움말
```

> `DEVELOPER_DIR=…` 접두는 `xcode-select`가 Command Line Tools를 가리키는 환경 대비입니다
> (`xcodebuild … requires Xcode` 오류 방지). Xcode가 이미 선택돼 있어도 무해합니다.
> 영구 전환을 원하면: `sudo xcode-select -s /Applications/Xcode.app/Contents/Developer`
>
> **Gatekeeper 안내**: ad-hoc 서명(미공증) 앱이라, 직접 빌드하지 않은 사본을 처음 열 때 경고가 뜨면 **우클릭 → "열기"** 로 실행하세요.

### 권한 안내
- **전체 디스크 접근**(선택): 휴지통 내용·일부 보호 폴더의 크기 측정에 필요 — 없으면 해당 항목은 "—"/"≥"로 정직하게 표시됩니다.
- **로컬 네트워크**(선택): 네트워크 보기에서 주변 컴퓨터를 찾는 데 사용합니다.

> **자주 재빌드하는 개발자용 참고**: ad-hoc 서명은 빌드마다 지문이 바뀌어 **전체 디스크 접근 승인이 재빌드 때마다 풀립니다**.
> 키체인 접근 ▸ 인증서 지원으로 자가 서명 코드 서명 인증서를 만들고 빌드 후
> `codesign --force --deep -s "<인증서 이름>" TreeFinder.app` 으로 재서명하면 승인이 유지됩니다.
> (한 번 빌드해서 쓰는 경우에는 해당 없음.)

---

## 🏗️ 설계 노트

- **순수 AppKit 프로그래매틱 UI** — 스토리보드/XIB 없음(설정 화면만 SwiftUI Form)
- **외부 의존성 최소** — SPM 의존성은 SwiftTerm 하나. rhwp(HWP 렌더러 CLI)와 Toast UI Editor는 라이선스 고지와 함께 리소스로 동반
- **폴더 크기 = 단일 actor 소유** — 어떤 패널도 자체 스캔을 시작하지 않고 조회만(중복 스캔·TCC 프롬프트 폭주 방지), 값의 역행 없는 스트리밍 표시
- **파일 연산 엔진** — 백그라운드 실행 + 진행률 + 항목 경계 취소, 모든 조작에 Undo 등록
- **비샌드박스** — 범용 파일 매니저 특성상 샌드박스와 양립 불가. 네트워크 자격 증명은 앱이 저장하지 않고 전부 키체인에 위임

---

## 🧩 서드파티

| 구성 요소 | 용도 | 라이선스 |
|---|---|---|
| [SwiftTerm](https://github.com/migueldeicaza/SwiftTerm) 1.15.0 | 내장 터미널 | MIT (Miguel de Icaza · xterm.js authors · SourceLair · Christopher Jeffrey) |
| [rhwp](https://github.com/edwardkim/rhwp) 0.7.18 | HWP/HWPX 페이지 렌더(동반 CLI) | MIT — 정적 링크된 Rust 크레이트 195개(전부 허용적 라이선스)는 고지 파일에 표로 수록 |
| [Toast UI Editor](https://ui.toast.com/tui-editor) 3.2.2 | 마크다운 WYSIWYG 편집 | MIT — 번들 내 ProseMirror(MIT)·DOMPurify(Apache-2.0) 포함 |

전체 고지문(라이선스 원문 전재)은 [TreeFinder/Vendor/THIRD-PARTY-NOTICES.txt](TreeFinder/Vendor/THIRD-PARTY-NOTICES.txt)에 있으며, 앱 안에서는 **도움말 ▸ 오픈 소스 라이선스**로 볼 수 있습니다.

---

## 📝 변경 이력

버전별 상세 내용은 [CHANGELOG.md](CHANGELOG.md) 및 [Releases](https://github.com/LimMinGue/TreeFinder/releases)를 참고하세요.

---

## 📄 라이선스

**독점 라이선스**(Proprietary) — © 2026 LimMinGue. All rights reserved. 자세한 조건은 [LICENSE](LICENSE)를 참고하세요.

- **v0.4.6 이하**(2026-08-07 이전 공개분)는 해당 버전에 한해 계속 [MIT 라이선스](https://github.com/LimMinGue/TreeFinder/blob/v0.4.6/LICENSE)가 적용됩니다.
- 동반 오픈 소스 구성 요소는 각자의 라이선스를 따릅니다([서드파티](#-서드파티) 참조).

버그 리포트 및 문의: iamwhatiam78@gmail.com
